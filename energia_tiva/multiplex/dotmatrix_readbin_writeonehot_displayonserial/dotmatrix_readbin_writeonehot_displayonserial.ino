...
This code did not function as well as initially hoped. This was due to a misconfiguration and 
misunderstanding of the testbench hardware configuration. Some LEDs were dimmer than other 
lookalikes, and as a result, time was spent troubleshooting that hardware rather than the 
software driving it. Ultimately the assortment of LEDs had some that were just dim under 
the same load and nothing was wrong. This led to the reconfiguration of the code to perform 
serial writes instead of GPIO toggles, to ensure nothing was algorithmically incorrect 
